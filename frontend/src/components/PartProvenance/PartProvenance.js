...
export const PartProvenance = ({ parts, refresh }) => {
  ...
  React.useEffect(() => {
    if (parts.join("") === "demoPart") {
      setPartData([
        {
          description: { id: "pièce de démonstration", name: "Pièce de démonstration hors réseau" },
          totalHours: 100,
          maximumHours: 3000,
          history: [
            {
              ...
              {part.history && (
                <React.Fragment>
                  <ProgressBar
                    ...
                    label={`Avion actuel: ${part.history[part.history.length - 1].tailNumber}`}
                  />
                  <HistoryTable
